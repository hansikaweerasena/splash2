#line 185 "/home/subodha/resources-gem5/benchmarks/splash2/codes/null_macros/c.m4.null.POSIX_BARRIER"
...
void startrun()
{
   ...
   if (*infile != '\0'/*NULL*/) {
      ...
      nbody = 256;//getiparam("nbody");
      if (nbody < 1) {
	 error("startrun: absurd nbody\n");
      }
      seed = 123;//getiparam("seed");
   }

   outfile = "";//getparam("out");
   dtime = 0.025;//getdparam("dtime");
   dthf = 0.5 * dtime;
   eps = 0.05;//getdparam("eps");
   epssq = eps*eps;
   tol = 1.0;//getdparam("tol");
   tolsq = tol*tol;
   fcells = 2.0;//getdparam("fcells");
   fleaves = 5.0;//getdparam("fleaves");
   tstop = 0.075;//getdparam("tstop");
   dtout = 0.25;//getdparam("dtout");
   NPROC = 1;//getiparam("NPROC");
   Local[0].nstep = 0;
   pranset(seed);
   testdata();
   ANLinit();
